...
const PhoneAnimation = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setTimeout(() => {
        setPlayKey((k) => k + 1);
        setShouldPlay(true);
      }, 50); // tiny gap to trigger CSS animation restart
    }, 12000);
  ...
  return (
    <div ref={ref} className={`w-full h-[400px] flex justify-center items-center relative overflow-visible pointer-events-none transition-all duration-1000 ${shouldPlay ? 'opacity-100' : 'opacity-0'}`}>
      
      <style>{`
        .custom-phone-scale {
          transform: translate(-50%, -50%) scale(0.748);
        }
        @media (min-width: 640px) {
          .custom-phone-scale {
            transform: translate(-50%, -50%) scale(0.816);
          }
        }
      `}</style>
      ...
      */}
      <div key={playKey} className={`absolute top-1/2 left-1/2 origin-center shrink-0 custom-phone-scale ${shouldPlay ? 'play-animation' : ''}`}>
        
        {/* Phone Body */}
        <div className="relative w-64 h-[500px] lg:h-[500px] bg-slate-900 rounded-[3rem] border-4 border-slate-800 shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden transform transition hover:scale-105 duration-500">
          ...
          <div className="absolute inset-0 flex flex-col pt-10 pb-6 px-4 bg-slate-900/95 z-10">
            <div className="flex flex-col items-center mb-4">
              <div className="w-24 h-24 rounded-full bg-brand-dark border-2 border-brand-neon/50 flex items-center justify-center shadow-[0_0_30px_rgba(14,165,233,0.4)] mb-3 relative overflow-hidden">
                <img 
            ...
            {/* Audio Waveform */}
            <div className="active-element-1 flex justify-center gap-1 h-6 mb-4 opacity-0 shrink-0">
              {[0, 0.1, 0.2, 0.3, 0.4].map((delay, i) => (
                <div key={i} className="w-1 bg-brand-neon rounded-full animate-wave" style={{animationDelay: `${delay}s`}}></div>
              ))}
            </div>

            {/* Chat Bubbles and Notifications */}
            <div className="space-y-3 w-full">
              ...
              {/* Confirmation Toast */}
              <div className="active-element-4 bg-green-500/20 border border-green-500 p-2.5 rounded-xl flex items-center gap-3 opacity-0 shadow-lg backdrop-blur-md">
